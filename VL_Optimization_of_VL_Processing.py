...
class VL_OPTIMIZATION_OF_VL_PROCESSING():
    def __init__( self, INFORMATION, MESSAGE_TYPE, NET_TYPE ):
        ...
        self.DELAY_BOUND = INFORMATION[6]
        self.DELAY_OCCURRED = INFORMATION[7]
        self.MESSAGE_TYPE = MESSAGE_TYPE
        self.NET_TYPE = NET_TYPE
        print("Messages type:", self.MESSAGE_TYPE)
        ...
        print("GUID_OF_MESSAGE", self.GUID_OF_MESSAGE)
        ...
        print("self.DELAY_BOUND:", self.DELAY_BOUND)
        print("self.DELAY_OCCURRED:", self.DELAY_OCCURRED)
        print("Length of LOGICAL_DESTINATION_OF_MESSAGE:", len(self.LOGICAL_DESTINATION_OF_MESSAGE))
    ...
    def vl_of_rdiu_with_period(self, GAP, TIMELIMITED, PHYSICAL_PORT_RATE):
        ...
        BANDWIDTH_COST = md.addVar( vtype=GRB.INTEGER, name="BANDWIDTH_COST" ) # 耗费（占用）的带宽
        ...
        md.addConstr( BANDWIDTH_COST == quicksum(BANDWIDTH_OF_EXISTING_VL[virtual_link_index] for virtual_link_index in range(NUMBER_OF_MESSAGES)) )
        md.setObjective( BANDWIDTH_COST, GRB.MINIMIZE)
        ...
        virtual_links = []
        for virtual_link_index in range(NUMBER_OF_MESSAGES):
            if VIRTUAL_LINKS[virtual_link_index].x >= Hyper_Parameters.threshold_for_binary:
                virtual_links.append(1)
            else:
                virtual_links.append(0)

        belong = []
        for message_index in range(NUMBER_OF_MESSAGES):
            intermediate = []
            for virtual_link_index in range(NUMBER_OF_MESSAGES):
                if BELONG[message_index, virtual_link_index].x >= Hyper_Parameters.threshold_for_binary:
        ...
        frame_size_of_vl = []
        for virtual_link_index in range(NUMBER_OF_MESSAGES):
            frame_size_of_vl.append(int(FRAME_SIZE_OF_VL[virtual_link_index].x))

        bandwidth_of_vl = []
        for virtual_link_index in range(NUMBER_OF_MESSAGES):
            bandwidth_of_vl.append(BANDWIDTH_OF_VL[virtual_link_index].x)

        size_of_vl = [] # 计算各条虚拟链路上，消息的大小（因为消息经过合并，所以大小值为：该条虚拟链路上消息的大小之和）
        for virtual_link_index in range( NUMBER_OF_MESSAGES ):
            if virtual_links[virtual_link_index] != 1:
                ...
                for message_index in range( NUMBER_OF_MESSAGES ):
                    if belong[ message_index ][ virtual_link_index ] == 1:
                        size += self.SIZE_OF_MESSAGE[ message_index ]
                size_of_vl.append( size )

        bandwidth_cost = BANDWIDTH_COST.x
        ...
        for virtual_link_index in range(NUMBER_OF_MESSAGES):
            if virtual_links[ virtual_link_index ] == 1:
                ...
                delay_bound_of_sub_virtual_link, delay_occurred_of_sub_virtual_link = [], []
                for message_index in range( len( belong ) ):
                    if belong[ message_index ][ virtual_link_index ] == 1:
                        ...
                        delay_bound_of_sub_virtual_link += self.DELAY_BOUND[ message_index ]
                        for index in range(len(self.DELAY_OCCURRED[message_index])):
                            self.DELAY_OCCURRED[ message_index ][ index ] += 5 # 非ARINC664消息进入RDIU前的传输时延，这里取了一个极坏值，而不是一个真实值
                            self.DELAY_OCCURRED[ message_index ][ index ] += ( math.ceil( size_of_vl[ virtual_link_index ]/frame_size_of_vl[ virtual_link_index ] ) - 1 ) * bag_of_vl[ virtual_link_index ]
                        delay_occurred_of_sub_virtual_link +=  self.DELAY_OCCURRED[ message_index ]
                ...
                delay_bound = [ delay_bound_of_sub_virtual_link, [], [], [] ]
                delay_occurred = [ delay_occurred_of_sub_virtual_link, [], [], [] ]
                vl_information.append( messages_guid )
                vl_information.append( logical_destination )
                vl_information.append( physical_destination )
                vl_information.append( delay_bound )
                vl_information.append( delay_occurred )
                VL_INFORMATION.append( vl_information )
        return VL_INFORMATION, bandwidth_cost
    ...
    def vl_of_end_system(self, GAP, TIMELIMITED, PHYSICAL_PORT_RATE):
        ...
        for virtual_link_index in range(NUMBER_OF_MESSAGES):
            intermediate = []
            for sub_virtual_link_index in range(NUMBER_OF_SUB_VIRTUAL_LINKS):
                if SUB_VIRTUAL_LINKS[virtual_link_index, sub_virtual_link_index].x >= Hyper_Parameters.threshold_for_binary:
                    intermediate.append(1)
                else:
                    intermediate.append(0)
        ...
        for message_index in range(NUMBER_OF_MESSAGES):
            ...
            for virtual_link_index in range(NUMBER_OF_MESSAGES):
                intermediate_inner = []
                for sub_virtual_link_index in range(NUMBER_OF_SUB_VIRTUAL_LINKS):
                    if BELONG[message_index, virtual_link_index, sub_virtual_link_index].x >= Hyper_Parameters.threshold_for_binary:
                        intermediate_inner.append(1)
                    else:
                        intermediate_inner.append(0)
                ...
                for sub_virtual_link_index in range( NUMBER_OF_SUB_VIRTUAL_LINKS ):
                    ...
                    for message_index in range( NUMBER_OF_MESSAGES ):
                        if belong[ message_index ][ virtual_link_index ][ sub_virtual_link_index ] == 1:
                            ...
                            delay_bound_of_sub_virtual_link += self.DELAY_BOUND[message_index]
                            for index in range( len( self.DELAY_OCCURRED[message_index] ) ):
                                self.DELAY_OCCURRED[message_index][index] += ( math.ceil(self.SIZE_OF_MESSAGE[message_index]/frame_size_of_vl[virtual_link_index])-1 ) * bag_of_vl[virtual_link_index]
                            delay_occurred_of_sub_virtual_link += self.DELAY_OCCURRED[message_index]
                    ...
                    delay_bound.append( delay_bound_of_sub_virtual_link )
                    delay_occurred.append( delay_occurred_of_sub_virtual_link )
                vl_information.append( messages_guid )
                vl_information.append( logical_destination )
                vl_information.append( physical_destination )
                vl_information.append( delay_bound )
