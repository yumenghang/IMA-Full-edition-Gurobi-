...
SAVE_FILE = Hyper_Parameters.intermediate_data_file
CAN_MESSAGE_SIZE = Hyper_Parameters.can_message_size
ARINC_429_MESSAGE_SIZE = Hyper_Parameters.arinc429_message_size
Analog_MESSAGE_SIZE = Hyper_Parameters.analog_message_size
SYSTEM_LATENCY_UPPER_LIMIT = Hyper_Parameters.system_latency_upper_limit

class CREATE_AND_DELETE_SAVE_FILE():
    """
    每次重新处理原始数据文件时，首先检查是否存在路径SAVE_FILE:
    1, 若存在：将SAVE_FILE删除，并重新创建SAVE_FILE
    2, 若不存在：创建SAVE_FILE
    """
    def create_and_deleta_save_file(self):
        isExists = os.path.exists(SAVE_FILE)
        if not isExists:
            os.makedirs(SAVE_FILE)
            print("%s has been successfully created!" % SAVE_FILE)
        else:
            shutil.rmtree(SAVE_FILE)
            print("%s has been successfully fully deleted!" % SAVE_FILE)
            os.makedirs(SAVE_FILE)
            print("%s has been successfully created!" % SAVE_FILE)
        ...
        for logical_port_node in logical_port_nodes:
            ...
            if logical_port_node.childNodes[1].nodeName == "Ep_Ref":  # 此类型消息无需考虑
                ...
                if logical_port_node.nodeName in ["A653SamplingPort", "A653QueuingPort", "HFSamplingPort", "HFQueuingPort"]:
                    ...
                    message_type = "CAN"
                    message_size = CAN_MESSAGE_SIZE  # 消息大小
                    ...
                    message_type = "A429"
                    message_size = ARINC_429_MESSAGE_SIZE  # 消息大小
                    physical = [logical_port_node.getAttribute("Physical")]  # 消息的发送物理端口名称
                    TransmissionIntervalMinimum = tx_port_attribute[logical_port_node.getAttribute("GuidDef")][1]  # 消息的发送周期
                elif logical_port_node.nodeName == "AnalogPort":
                    message_type = "Analog"
                    message_size = Analog_MESSAGE_SIZE  # 消息大小
                    physical = [logical_port_node.getAttribute("Physical")]  # 消息的发送物理端口名称
                    TransmissionIntervalMinimum = tx_port_attribute[logical_port_node.getAttribute("GuidDef")][1]  # 消息的发送周期
                else:
            ...
            if logical_port_node.nodeName in ["A653SamplingPort", "A653QueuingPort", "HFSamplingPort", "HFQueuingPort"]:
                ...
                if logical_port_node.childNodes[1].nodeName == "Ep_Ref":  # 此类型消息无需考虑
                    ...
                    if RP_node.getAttribute("SysLatencyWCLimit") == "0":
                        syslantencywclimit = SYSTEM_LATENCY_UPPER_LIMIT
                    else:
                ...
                if logical_port_node.nodeName == "CANPort":
                    message_size = CAN_MESSAGE_SIZE
                elif logical_port_node.nodeName == "A429Port":
                    message_size = ARINC_429_MESSAGE_SIZE
                else:
                    message_size = Analog_MESSAGE_SIZE
                #因为<Ep_Ref>, <Emb_Ref>, <Reversion_Ref>均是回填回去的，因此若存在<RP>标签，则一定在<Ep_Ref>, <Emb_Ref>, <Reversion_Ref>这三个标签之前
                if logical_port_node.childNodes[1].nodeName == "Ep_Ref":  # 此类型消息无需考虑
                    continue
                elif logical_port_node.childNodes[1].nodeName == "Emb_Ref":  # 此类型消息无需考虑
            ...
            if self.messages_info[message_guid][0] == "A664":  # ARINC664消息
                for physical_port_name in self.messages_info[message_guid][4]:  # 因为消息可能从A, B网同时进行传输，所以这里消息的物理发送端口是一列表，其中有可能包括两个端口
                    if physical_port_name in messages_per_physical_port:
                        ...
                        messages_per_physical_port[physical_port_name][5].append(message_guid)
                        logical_destination_of_message, physical_destination_of_message = [], []
                        delay_bound, delay_occurred = [], []
                        for index in range(len(self.messages_info[message_guid][9])):
                            logical_destination_of_message.append( self.messages_info[message_guid][9][index][6] )
                            physical_destination_of_message.append( self.messages_info[message_guid][9][index][4] )
                            delay_bound.append( self.messages_info[message_guid][9][index][2] )
                            delay_occurred.append( 0 )
                        messages_per_physical_port[physical_port_name][6].append(logical_destination_of_message)
                        messages_per_physical_port[physical_port_name][7].append(physical_destination_of_message)
                        messages_per_physical_port[physical_port_name][8].append(delay_bound)
                        messages_per_physical_port[physical_port_name][9].append(delay_occurred)
                    else:
                        message_type, message_size = ["A664"], [self.messages_info[message_guid][1]]
                        guid_of_messages = [message_guid]
                        min_latency = self.messages_info[message_guid][9][0][2]
                        for index in range(len(self.messages_info[message_guid][9])):
                            min_latency = min(min_latency, self.messages_info[message_guid][9][index][2])
                        ...
                        for index in range(len(self.messages_info[message_guid][9])):
                            ...
                            delay_bound.append( self.messages_info[message_guid][9][index][2] )
                            delay_occurred.append( 0 )
                        messages_per_physical_port[physical_port_name] = [physical_port_name, message_type, message_size, message_latency, TransmissionIntervalMinimum, guid_of_messages, [ logical_destination_of_message ], [ physical_destination_of_message ], [ delay_bound ], [ delay_occurred ] ]
            else:  # 非ARINC664消息
                total = 0
                        ...
                        if physical_ports_adjacent_matrix[physical_ports_index[physical_port_name]][index] == 1:  # 说明此非664消息经物理端口physical_port_name向RDIU中下标为index的物理端口传输数据
                            if physical_ports_information[physical_ports_index_reversed[index]][6][0:4] == "RDIU":  # 下标为index的物理端口所在物理设备为RDIU
                                ...
                                if RDIU_name in messages_per_physical_port:
                                    ...
                                    messages_per_physical_port[RDIU_name][5].append(message_guid)
                                    logical_destination_of_message, physical_destination_of_message = [], []
                                    delay_bound, delay_occurred = [], []
                                    for index in range(len(self.messages_info[message_guid][9])):
                                        logical_destination_of_message.append( self.messages_info[message_guid][9][index][6] )
                                        ...
                                        delay_bound.append(self.messages_info[message_guid][9][index][2])
                                        delay_occurred.append(0)
                                    messages_per_physical_port[RDIU_name][6].append( logical_destination_of_message )
                                    messages_per_physical_port[RDIU_name][7].append( physical_destination_of_message )
                                    messages_per_physical_port[RDIU_name][8].append( delay_bound )
                                    messages_per_physical_port[RDIU_name][9].append( delay_occurred )
                                else:
                                    ...
                                    for index in range(len(self.messages_info[message_guid][9])):
                                        ...
                                        delay_bound.append(self.messages_info[message_guid][9][index][2])
                                        delay_occurred.append(0)
                                    messages_per_physical_port[RDIU_name] = [RDIU_name, message_type, message_size, message_latency, TransmissionIntervalMinimum, guid_of_messages, [ logical_destination_of_message ], [ physical_destination_of_message], [ delay_bound ], [ delay_occurred ] ]
...
class SAVE_INTERMEDIATE_FILE:
    def save_file(self,
        ...
        np.save(SAVE_FILE+'messages_per_physical_port.npy', messages_per_physical_port)
        #读取方式
